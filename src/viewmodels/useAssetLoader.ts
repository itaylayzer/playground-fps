    ...
    loadMeshes(items) {
        ...
        return new Promise<void>((resolve, reject) => {
                    ...
                    case itemSrc.endsWith(".fbx"):
                        {
                            fbxLodaer.load(
                                ...
                                progres => {
                                    minerProgress =
                                        progres.loaded / progres.total;
                                    set({
                                        progress:
                                            itemProgress +
                                            minerProgress / itemsLength,
                                    });
                                },
                                error => {
                                    reject(error);
            ...
            loadingManager.onLoad = () => {
                console.log(
                    `Loaded FBX`,
                    JSON.stringify(Array.from(Object.keys(_assets.fbx)))
                );
                console.log(
                    `Loaded GLTF`,
                    JSON.stringify(Array.from(Object.keys(_assets.gltf)))
                );
                console.log(
                    `Loaded Textures`,
                    JSON.stringify(Array.from(Object.keys(_assets.textures)))
                );
                console.log(
                    `Loaded Fonts`,
                    JSON.stringify(Array.from(Object.keys(_assets.fonts)))
                );
