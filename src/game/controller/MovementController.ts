...
export class MovementController {
    ...
    update(group: THREE.Group,) {
        ...
        if ((Global.keyboardController.isKeyPressed("Space") ||
            Global.keyboardController.isKeyDown("Space")) && onGround) {
            onGround && (this.body.velocity.y = 11);
            this.onGroundController.off();
        }

        this.keysAxis[RAW_AXIS][VERTICAL] = +Global.keyboardController.isKeyPressed("KeyW") + -Global.keyboardController.isKeyPressed("KeyS");
        this.keysAxis[RAW_AXIS][HORIZONTAL] = -Global.keyboardController.isKeyPressed("KeyD") + +Global.keyboardController.isKeyPressed("KeyA");

        for (let index = 0; index < 2; index++) {
            this.keysAxis[LERPED_AXIS][index] = lerp(this.keysAxis[LERPED_AXIS][index], this.keysAxis[RAW_AXIS][index], Global.deltaTime * 8);
        }

        const yRotation = Global.cameraController.rotation.x * THREE.MathUtils.DEG2RAD;

        group.rotation.set(0, 0, 0);
        group.rotateOnWorldAxis(new THREE.Vector3(0, 1, 0), yRotation);

        const forwardVector = new CANNON.Vec3(Math.sin(yRotation), 0, Math.cos(yRotation));
        const rightVector = new CANNON.Quaternion()
            .setFromEuler(0, Math.PI / 2, 0)
            .vmult(new CANNON.Vec3(Math.sin(yRotation), 0, Math.cos(yRotation)));

        forwardVector.scale(this.keysAxis[LERPED_AXIS][VERTICAL], forwardVector);
        rightVector.scale(this.keysAxis[LERPED_AXIS][HORIZONTAL], rightVector);

        //@ts-ignore
        this.body.quaternion.copy(group.quaternion);
        const direction = forwardVector.vadd(new CANNON.Vec3().copy(rightVector));
